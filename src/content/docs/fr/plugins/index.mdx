...
description: Comprendre ce qu'une extension et un bloc sont, et apprendre comment en organiser à partir d'un fichier vide.
sidebar:
    order: 1
---
...
En raison de cette différence de finalité, les extensions sont gérées depuis l’écran d’administration des Extensions, tandis que les blocs sont gérés depuis l’écran d’administration des Blocs. En réalité, un bloc est une extension : une extension qui se déclare comme tel, comme vous le verrez ci-après.

## Tout commence par un simple fichier PHP 

Que vous développiez une extension ou un bloc, tout commence par un fichier PHP principal — celui que Retraceur chargera en premier, souvent appelé fichier « boot ». Ce fichier doit contenir un commentaire d'en-tête décrivant votre projet : son nom, sa version, son responsable, les versions de Retraceur qu'il prend en charge, etc.

![Administration des extensions](../../../../assets/images/build-plugins-header-tags-fr.png)

Ce commentaire d'en-tête est ce que Retraceur examine pour lister votre projet dans l'écran d'administration de votre site. C'est tout ce qui est nécessaire pour démarrer : dès qu'un commentaire d'en-tête valide est trouvé et analysé, votre extension ou votre bloc apparaissent et sont prêts à être activés.

Voici un commentaire d'en-tête minimaliste valide pour une extension :

<Code code={pluginCode} lang="php" title={pluginFileName} />

Pour la transformer en bloc à la place, il suffit d'ajouter cette ligne dans le commentaire : `Plugin Type: block`.
...
| **CODE_OF_CONDUCT.md** | Explique comment les autres doivent se comporter lorsqu'ils·elles apportent leur contribution. |
