...
Delete all files but the `/wp-content` directory and the `wp-config.php` & `.htaccess` files. Once done, delete the `/wp-content/themes/point` directory which contains the default Retraceur theme that is bundled inside Retraceur. Your Website files tree should look like:
...
## Step 8: Install updated Plugins and Themes
